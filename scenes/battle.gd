...
func _process(delta: float) -> void:
        ...
        # تحديث النصوص
        var enemy_pct: float = 0.0
        if data["enemy_power"] > 0:
                enemy_pct = (data["enemy_current_hp"] / data["enemy_power"]) * 100
        var player_pct: float = 0.0
        if data["player_power"] > 0:
                player_pct = (data["player_current_hp"] / data["player_power"]) * 100
